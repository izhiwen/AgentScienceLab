# AgentScienceLab — Codex Adapter

## Current state in v0.1.x

This directory is **intentionally minimal in v0.1**. ASL's role
...
- A Codex-specific `SKILL.md` for the ASL team (so Codex's skill picker
  surfaces "ASL: route to Theorist" / "ASL: fire LLM-Measurement
...
- Codex-specific managed block templates for `AGENTS.md` so the
  ASL roles appear as a discoverable virtual team in long sessions
...
manifest gives each ASL role a routing description that Codex
...
Everything an ASL user needs today works through `aiplus agent
