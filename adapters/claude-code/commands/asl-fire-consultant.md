# /asl-fire-consultant — Fire the ASL research consultant team
...
1. Read `.aiplus/consultant-team.toml` to see who is seated. ASL's
   research-tuned config replaces AiPlus's default SWE consultant team
   when `aiplus add agentsciencelab` was run.
...
```text
/asl-fire-consultant 用 staggered DID 还是 imputation 估计器 → HEAVY
/asl-fire-consultant 把 LLM 评分作为主结果是否站得住 → llm-measurement + econometrician + referee
/asl-fire-consultant 数据共享方案 → ethics-irb + reproducibility + coauthor-liaison
```
