# /asl-route — Route a task to the right ASL role

Use this command to explicitly route work through the ASL coordinator
...
```text
/asl-route 帮我清一下 NLSY97 这份数据 → ra-python (LIGHT)
/asl-route 主回归用谁的 DID 估计量比较稳 → theorist + econometrician (MEDIUM)
/asl-route 投稿前挑刺一下 → referee + econometrician + viz-specialist (HEAVY)
```
