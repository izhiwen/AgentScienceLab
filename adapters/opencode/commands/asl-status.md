# /asl-status — Show ASL team status

Use this command to get a one-shot snapshot of the ASL research team in
this project.
...
4. Show in-flight tasks if ASL is tracking them via `aiplus agent
   transcript`.
...
```text
/asl-status
/asl-status pi          # narrow to just PI
/asl-status experts     # narrow to expert table
```
