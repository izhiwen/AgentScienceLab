...
AgentScienceLab (ASL) is project-local by default. All agent state, personas,
memories, and worktrees live inside `.aiplus/` of the project where ASL
is installed. ASL never uploads data, never syncs to a cloud, and never
edits global agent configuration.

## Data boundaries

ASL is the AiPlus sub-module for applied-economics research workflows.
Research data raises specific concerns that ASL takes seriously:

- **Restricted / IRB-protected data** MUST live outside any path ASL
  manages. ASL provides the **Ethics / IRB Reviewer expert** and the
  **IRB / Disclosure Gate** consultant seat specifically to gate tasks
  ...
  authorization memo. See ASL DESIGN.md §15 (Privacy & Safety
...
- **No PII**, no respondent identifiers, no IRB-restricted paths
  ...
  any record is written; ASL inherits this protection.
- **AEA Data Editor replication packages**: ASL's
...
ASL never auto-approves these actions, regardless of which agent is
driving:
...
See ASL DESIGN.md §16 for the full STOP-gate list.

## Reporting a security or privacy issue

ASL is brand-decoupled from AiPlus but functionally depends on the
...
Issues specific to ASL's research-tuned consultant team, the LLM-as-
...
For sensitive disclosures (e.g. you found a way for ASL to leak
restricted data outside an authorized path), please use email rather
than a public issue.

## What ASL does not do
