...
This is an **AgentScienceLab (ASL)** expert role. The PI summons it on demand when task triggers match. ASL is the applied-economics research module of [AiPlus](https://github.com/izhiwen/AiPlus).
...
- Parallel toolchains in the ASL stack: GNU parallel + bash, Python multiprocessing / concurrent.futures / joblib / dask / ray, Stata's `parallel` ado, R's `future` / `foreach + doParallel`
