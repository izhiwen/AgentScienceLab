...
asserts that every installed runtime has its expected AgentScienceLab artifacts.
"""
...
ASL_AGENT_NAMES = CORE_ROLES + EXPERTS
ASL_COMMANDS = [
...
CASES = {
    "h1_codex_ael_then_claude": {
        "description": "codex + agentsciencelab -> install claude-code (H1)",
        "steps": [
            ("install", "codex"),
            ("add", "agentsciencelab"),
            ("install", "claude-code"),
    ...
    "codex_ael_then_opencode": {
        "description": "codex + agentsciencelab -> install opencode",
        "steps": [
            ("install", "codex"),
            ("add", "agentsciencelab"),
            ("install", "opencode"),
    ...
    "claude_ael_then_codex": {
        "description": "claude-code + agentsciencelab -> install codex",
        "steps": [
            ("install", "claude-code"),
            ("add", "agentsciencelab"),
            ("install", "codex"),
    ...
    "claude_ael_then_opencode": {
        "description": "claude-code + agentsciencelab -> install opencode",
        "steps": [
            ("install", "claude-code"),
            ("add", "agentsciencelab"),
            ("install", "opencode"),
    ...
    "opencode_ael_then_codex": {
        "description": "opencode + agentsciencelab -> install codex",
        "steps": [
            ("install", "opencode"),
            ("add", "agentsciencelab"),
            ("install", "codex"),
    ...
    "opencode_ael_then_claude": {
        "description": "opencode + agentsciencelab -> install claude-code",
        "steps": [
            ("install", "opencode"),
            ("add", "agentsciencelab"),
            ("install", "claude-code"),
    ...
    "all_runtimes_then_ael": {
        "description": "install all 3 runtimes -> add agentsciencelab last",
        "steps": [
            ("install", "codex"),
            ("install", "claude-code"),
            ("install", "opencode"),
    ...
    "claude_ael_then_codex_then_opencode": {
        "description": "install claude-code + add agentsciencelab + 2 others in series",
        "steps": [
            ("install", "claude-code"),
            ("add", "agentsciencelab"),
            ("install", "codex"),
    ...
    "codex_ael_then_claude_then_opencode": {
        "description": "install codex + add agentsciencelab + 2 others in series",
        "steps": [
            ("install", "codex"),
            ("add", "agentsciencelab"),
            ("install", "claude-code"),
...
def require_core_ael_files(root: Path) -> None:
    require_file(root, ".aiplus/modules/agentsciencelab/aiplus-module.json")
...
def require_markdown_agent_dir(root: Path, runtime_dir: str) -> None:
    ...
    expected = {f"agentsciencelab-{name}.md" for name in ASL_AGENT_NAMES}
    actual = {path.name for path in agent_dir.glob("agentsciencelab-*.md")}
    if actual != expected:
...
def require_markdown_command_dir(root: Path, runtime_dir: str) -> None:
    ...
    actual = {path.name for path in command_dir.glob("asl-*.md")}
    expected = set(ASL_COMMANDS)
    if actual != expected:
        missing = sorted(expected - actual)
        extra = sorted(actual - expected)
...
def run_case(case_id: str, bin_path: Path, keep_project: bool) -> None:
    ...
    project = Path(tempfile.mkdtemp(prefix=f"asl-order-{case_id}-", dir=temp_root))
    ...
    try:
        ...
        if "agentsciencelab" not in modules:
            fail("manifest missing agentsciencelab module")
