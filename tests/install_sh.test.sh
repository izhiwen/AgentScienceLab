...
case "$(/usr/bin/uname -s):$(/usr/bin/uname -m)" in
  ...
  *)
    ...
    case "$real_drop_out" in
      *"irm https://raw.githubusercontent.com/izhiwen/AgentScienceLab/main/install.ps1 | iex"*) ;;
      *)
...
case "$(uname -s):$(uname -m)" in
  Darwin:arm64) ;;
  *)
    echo "ASL_INSTALL_SH_TEST=PASS (negative path only; positive path skipped on $(uname -s)-$(uname -m))"
    exit 0
    ;;
esac
...
dry_run="$(
  ASL_RELEASES_LATEST_URL="file://$latest_root/tag/v9.9.9/index.html" \
...
fallback_output="$(
  ASL_RELEASES_LATEST_URL="file://$latest_root/missing" \
  sh install.sh --dry-run 2>&1
)"
case "$fallback_output" in
  *"WARNING could not resolve latest ASL release"*) ;;
  *)
...
package_root="$tmp/pkg/asl-v9.9.9-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)"
mkdir -p "$release_dir" "$package_root/bin" "$package_root/libexec"

cat >"$package_root/bin/asl" <<'SH'
#!/usr/bin/env bash
echo "fake asl"
SH
cat >"$package_root/libexec/asl-support" <<'SH'
#!/usr/bin/env bash
echo "fake support"
SH
chmod +x "$package_root/bin/asl" "$package_root/libexec/asl-support"
...
asset="asl-v9.9.9-$os-$arch.tar.gz"
...
output="$(
  ASL_VERSION=v9.9.9 \
  ASL_BASE_URL="file://$release_dir" \
  ASL_INSTALL_DIR="$install_root/bin" \
  ASL_LIBEXEC_DIR="$install_root/libexec" \
  sh install.sh
)"
...
[ -x "$install_root/bin/asl" ] || {
  echo "::error::asl wrapper not installed"
  exit 1
}
[ -x "$install_root/libexec/asl-support" ] || {
