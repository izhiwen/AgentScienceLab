...
make_project() {
  ...
  printf '{"runtimeAdapters":["codex"],"modules":{"agentsciencelab":{"version":"test"}}}\n' \
...
bash -n asl
bash -n scripts/build-asl.sh

version="$(./asl --version)"
[ "$version" = "ASL 0.3.0 (aiplus 0.6.19+)" ] || fail "unexpected asl version output: $version"

help="$(./asl --help)"
for cmd in "asl install" "asl update" "asl uninstall" "asl doctor" "asl status" "asl refresh"; do
  case "$help" in
    *"$cmd"*) ;;
    *) fail "asl --help missing ASL-specific command: $cmd" ;;
  esac
done
case "$help" in
  *"telemetry"*)
    fail "asl --help must not advertise telemetry"
    ;;
esac
for role in pi advisor writer ra-stata ra-python theorist referee replicator pm; do
  case "$help" in
    *"asl $role"*) ;;
    *) fail "asl --help missing direct-shortcut entry for role: $role" ;;
  esac
done
...
assert_delegate() {
  ...
  case "$out" in
    *"AIPLUS_BRAND=ASL"*\
*"AIPLUS_TEAM=agentsciencelab"*\
*"AIPLUS_DEFAULT_ROLE=pi"*\
*"ARGS=$expected_args"*) ;;
    *)
      printf '%s\n' "$out"
      fail "unexpected delegate output for asl $*"
      ;;
  esac
}
...
chat_args_status=$?
set -e
[ "$chat_args_status" -ne 0 ] || fail "asl chat with arguments must fail"
case "$chat_args_out" in
  *"\`asl chat\` does not accept arguments"*\
*"asl \"...\""*) ;;
  *"ARGS="*)
    printf '%s\n' "$chat_args_out"
    fail "asl chat with arguments must not delegate to substrate"
    ;;
  *)
    printf '%s\n' "$chat_args_out"
    fail "asl chat with arguments error missing guidance"
    ;;
esac
...
telemetry_status=$?
set -e
[ "$telemetry_status" -ne 0 ] || fail "asl telemetry must be removed"
case "$telemetry_out" in
  *"asl telemetry has been removed"*) ;;
  *)
    printf '%s\n' "$telemetry_out"
    fail "asl telemetry removal error changed unexpectedly"
    ;;
esac
...
case "$stale_out" in
  *"team=agent-team, not agentsciencelab"*\
...
dry_run="$(./asl install codex --dry-run)"
case "$dry_run" in
  *"ASL install dry-run"*\
*"runtime=codex"*\
...
*"ASL_DRY_RUN=PASS"*) ;;
  *)
    printf '%s\n' "$dry_run"
    fail "asl install codex --dry-run changed unexpectedly"
    ;;
esac
...
status_out="$(cd "$status_project" && ASL_AIPLUS_BIN="$support_bin" "$ael_abs" status)"
case "$status_out" in
  *"SUPPORT_ARGS=agent status"*\
*"SUPPORT_BRAND=ASL"*) ;;
  *)
    printf '%s\n' "$status_out"
    fail "asl status must remain ASL-handled via support binary"
    ;;
esac

refresh_project="$(make_project)"
refresh_out="$(cd "$refresh_project" && ASL_AIPLUS_BIN="$support_bin" "$ael_abs" refresh --dry-run)"
case "$refresh_out" in
  *"SUPPORT_ARGS=refresh --dry-run"*\
*"SUPPORT_BRAND=ASL"*) ;;
  *)
    printf '%s\n' "$refresh_out"
    fail "asl refresh must delegate explicitly to substrate refresh"
    ;;
esac
...
newer_path_out="$(cd "$newer_path_project" && PATH="$newer_path_bin:$PATH" ASL_AIPLUS_BIN="$older_support_bin" "$ael_abs" doctor 2>&1)"
case "$newer_path_out" in
  *"NEEDS_FIX aiplus_version_mismatch"*)
    printf '%s\n' "$newer_path_out"
    fail "asl doctor must not downgrade-warn when PATH aiplus is newer than bundled support"
    ;;
  *"DOCTOR_STATUS=PASS"*) ;;
  *)
    printf '%s\n' "$newer_path_out"
    fail "asl doctor newer PATH smoke output missing expected doctor status"
    ;;
esac

ael_consultant_project="$(make_project)"
cp core/templates/consultant-team.agentsciencelab.toml "$ael_consultant_project/.aiplus/consultant-team.toml"
ael_consultant_out="$(
  cd "$ael_consultant_project" && \
    PATH="$newer_path_bin:$PATH" \
    ASL_AIPLUS_BIN="$older_support_bin" \
    "$ael_abs" doctor 2>&1
)"
case "$ael_consultant_out" in
  ...
  *)
    printf '%s\n' "$ael_consultant_out"
    fail "asl doctor must pass the ASL research consultant team config"
    ;;
esac
...
set -e
[ "$default_consultant_status" -ne 0 ] || fail "asl doctor must fail when ASL project has default SWE consultant config"
case "$default_consultant_out" in
  *"NEEDS_FIX ael_consultant_team_mismatch"*) ;;
  *)
    printf '%s\n' "$default_consultant_out"
    fail "asl doctor must flag default SWE consultant config under ASL"
    ;;
esac
...
update_pkg="$update_tmp/pkg/asl-v9.9.9"
mkdir -p "$update_install" "$update_libexec" "$update_release" "$update_pkg/bin" "$update_pkg/libexec"
cat >"$update_install/asl" <<'SH'
#!/usr/bin/env bash
printf 'ASL 0.3.0 (aiplus 0.6.19+)\n'
SH
cat >"$update_pkg/bin/asl" <<'SH'
#!/usr/bin/env bash
printf 'ASL 9.9.9 (aiplus 0.6.19+)\n'
SH
cat >"$update_pkg/libexec/asl-support" <<'SH'
#!/usr/bin/env bash
case "${1:-}" in
  --version)
    printf 'aiplus 0.6.20\n'
...
SH
chmod +x "$update_install/asl" "$update_pkg/bin/asl" "$update_pkg/libexec/asl-support"
...
update_asset="asl-v9.9.9-$update_os-$update_arch.tar.gz"
...
update_out="$(
  PATH="$newer_path_bin:$PATH" \
  ASL_UPDATE_LATEST_VERSION=9.9.9 \
  ASL_BASE_URL="file://$update_release" \
  ASL_INSTALL_DIR="$update_install" \
  ASL_LIBEXEC_DIR="$update_libexec" \
  "$ael_abs" update 2>&1
)"
case "$update_out" in
...
*"UPDATE_STATUS=PASS"*) ;;
  *)
    printf '%s\n' "$update_out"
    fail "asl update must not overwrite a newer PATH aiplus with older bundled support"
    ;;
esac
...
[ "$post_update_path_version" = "aiplus 0.7.5" ] || {
  ...
  fail "asl update downgraded PATH aiplus"
}
...
auto_out="$(
  cd "$no_manifest" && \
    ...
    ASL_AIPLUS_BIN="$auto_support" \
    ASL_SUPPORT_LOG="$auto_log" \
    "$ael_abs" pi
)"
case "$auto_out" in
  *"ASL set up for: codex"*\
...
*"add agentsciencelab"*\
*"agent set-team agentsciencelab"*\
*"mcp-register --runtime codex"*) ;;
  *)
    cat "$auto_log"
    fail "fresh role shortcut did not run ASL install flow"
    ;;
esac

echo "ASL_WRAPPER_TEST=PASS"
