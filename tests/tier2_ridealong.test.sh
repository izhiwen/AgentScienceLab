...
if manifest.get("name") != "agentsciencelab":
    fail("aiplus-module.json name must be agentsciencelab")
...
expected_subagent_names = {f"agentsciencelab-{role}" for role in expected_roles}
expected_personas = {
    f"agentsciencelab-{role}": f"core/templates/personas/{role}.md"
...
expected_new_names = {"agentsciencelab-dof-auditor", "agentsciencelab-rr-strategist"}
