...
Describe "bin/asl.ps1" {
BeforeAll {
$RepoRoot = Split-Path -Parent $PSScriptRoot
$Wrapper = Join-Path $RepoRoot "bin\asl.ps1"
...
function Read-TextFileOrEmpty {
  ...
  It "prints the Windows wrapper version" {
    ...
    $result.Output.Trim() | Should -Be "ASL 0.3.0 (aiplus 0.6.19+)"
  }
  ...
  It "rejects accidental multi-argument command paths" {
    ...
    $result.Output | Should -Match 'asl "\.\.\."'
  }

  It "keeps chat as a no-argument lobby alias only" {
    $result = Invoke-AelPs1 -Arguments @("chat", "advisor")
    $result.Status | Should -Not -Be 0
    $result.Output | Should -Match "asl chat does not accept arguments"
    $result.Output | Should -Match 'asl "\.\.\."'
  }
  ...
  It "delegates talk, role shortcuts, and freeform input like the Unix wrapper" {
    ...
    if ($isWindowsPlatform) {
      ...
      $support = Join-Path $fakeBin "asl-support.cmd"
...
'@ -Encoding ASCII
      ...
      chmod +x $runtime
      $support = Join-Path $fakeBin "asl-support"
      Set-Content -LiteralPath $support -Value "#!/usr/bin/env bash`nprintf '%s\n' ""`$*"" >""`$ASL_SUPPORT_LOG""`n" -Encoding ASCII
      chmod +x $support
    }
    ...
    $shortcutFresh = Invoke-AelPs1 -Arguments @("pi", "--fresh") -WorkingDirectory $project -Environment @{
      ASL_AIPLUS_BIN = $support
      ASL_SUPPORT_LOG = $shortcutFreshLog
      PATH = "$fakeBin$([IO.Path]::PathSeparator)$env:PATH"
    }
    ...
    $naturalLanguage = Invoke-AelPs1 -Arguments @("我想反思 RD 设计") -WorkingDirectory $project -Environment @{
      ASL_AIPLUS_BIN = $support
      ASL_SUPPORT_LOG = $naturalLanguageLog
      PATH = "$fakeBin$([IO.Path]::PathSeparator)$env:PATH"
    }
  ...
  It "delegates lobby and chat alias to substrate" {
    ...
    if ($isWindowsPlatform) {
      $support = Join-Path $fakeBin "asl-support.cmd"
      Set-Content -LiteralPath $support -Value "@echo off`r`nif ""%*""=="""" (`r`n  type nul > ""%ASL_SUPPORT_LOG%""`r`n) else (`r`n  echo %* > ""%ASL_SUPPORT_LOG%""`r`n)`r`nexit /b 0`r`n" -Encoding ASCII
    } else {
      $support = Join-Path $fakeBin "asl-support"
      Set-Content -LiteralPath $support -Value "#!/usr/bin/env bash`nprintf '%s\n' ""`$*"" >""`$ASL_SUPPORT_LOG""`n" -Encoding ASCII
      chmod +x $support
  ...
  It "installs all runtimes in sequence" {
    ...
    if ($isWindowsPlatform) {
      $support = Join-Path $fakeBin "asl-support.cmd"
      Set-Content -LiteralPath $support -Value "@echo off`r`necho %*>>%ASL_SUPPORT_LOG%`r`nexit /b 0`r`n" -Encoding ASCII
    } else {
      $support = Join-Path $fakeBin "asl-support"
      Set-Content -LiteralPath $support -Value "#!/usr/bin/env bash`nprintf '%s\n' ""`$*"" >>""`$ASL_SUPPORT_LOG""`n" -Encoding ASCII
      chmod +x $support
    ...
    $result.Status | Should -Be 0
    $result.Output | Should -Match "ASL install runtime=codex status=PASS"
    $result.Output | Should -Match "ASL install runtime=claude-code status=PASS"
    $result.Output | Should -Match "ASL install runtime=opencode status=PASS"
    $result.Output | Should -Match "ASL installed: codex PASS, claude-code PASS, opencode PASS"
    ...
    ($calls | Where-Object { $_ -eq "add agentsciencelab" }).Count | Should -Be 3
    ($calls | Where-Object { $_ -eq "agent set-team agentsciencelab" }).Count | Should -Be 3
  ...
  It "flags default SWE consultant config under an ASL project" {
    $project = Join-Path ([System.IO.Path]::GetTempPath()) ([System.Guid]::NewGuid().ToString("N"))
    $fakeBin = Join-Path ([System.IO.Path]::GetTempPath()) ([System.Guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Force -Path (Join-Path $project ".aiplus"), $fakeBin | Out-Null
    Set-Content -LiteralPath (Join-Path $project ".aiplus\manifest.json") -Value '{"runtimeAdapters":["codex"],"modules":{"agentsciencelab":{"version":"test"}}}' -Encoding UTF8
    ...
    Copy-Item -LiteralPath (Join-Path $RepoRoot "core\templates\consultant-team.agentsciencelab.toml") -Destination (Join-Path $project ".aiplus\consultant-team.toml") -Force
