#!/usr/bin/env python3
"""Headless TEST-3 harness for AgentScienceLab /asl-talk behavior.
...
def isolated_env(runtime: str, aiplus_bin: pathlib.Path) -> dict[str, str]:
    ...
    home = pathlib.Path(tempfile.mkdtemp(prefix=f"asl-test3-{runtime}-home-"))
    env["HOME"] = str(home)
...
def create_project(runtime: str, aiplus_bin: pathlib.Path, env: dict[str, str]) -> pathlib.Path:
    project = pathlib.Path(tempfile.mkdtemp(prefix=f"asl-test3-{runtime}-project-"))
    ...
    run([str(aiplus_bin), "add", "agentsciencelab"], cwd=project, env=env, timeout=180)
    run([str(aiplus_bin), "agent", "set-team", "agentsciencelab"], cwd=project, env=env, timeout=180)
    return project
...
def assert_persona_output(runtime: str, role: str, result: CommandResult) -> None:
    ...
    forbidden = ("unknown command", "command not found", "not found: /asl-talk", "unrecognized command")
    if any(token in lower for token in forbidden):
        print(output[-4000:], file=sys.stderr)
        raise SystemExit(f"{runtime}/{role} did not execute /asl-talk cleanly")
    missing = ["agentsciencelab"] if "agentsciencelab" not in lower else []
...
def role_prompt(role: str) -> str:
    return (
        ...
        "say that you are part of AgentScienceLab, and describe one concrete research "
...
def run_claude(project: pathlib.Path, roles: tuple[str, ...], env: dict[str, str]) -> None:
    ...
    model = env.get("ASL_CLAUDE_MODEL", "sonnet")
    for role in roles:
        result = run(
            [
                "claude",
                ...
                model,
                "--max-budget-usd",
                env.get("ASL_CLAUDE_MAX_BUDGET_USD", "0.25"),
                f"/asl-talk {role} {role_prompt(role)}",
            ],
            cwd=project,
            env=env,
            timeout=int(env.get("ASL_TEST3_TIMEOUT_SECONDS", "240")),
            check=False,
        )
...
def run_opencode(project: pathlib.Path, roles: tuple[str, ...], env: dict[str, str]) -> None:
    ...
    model = env.get("ASL_OPENCODE_MODEL", "openai/gpt-4o-mini")
    for role in roles:
        result = run(
            [
                "opencode",
