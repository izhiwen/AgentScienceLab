...
for file in "${public_files[@]}"; do
  ...
  if [ "$file" = "install.ps1" ] && grep -Ein '\bAiPlus\b|\baiplus\b|\bAIPLUS\b' "$file" | grep -Ev 'replace' >/tmp/asl-install-ps1-branding.txt; then
    echo "::error file=$file::public-facing substrate brand leak"
    cat /tmp/asl-install-ps1-branding.txt
    exit 1
  fi
done
...
grep -q 'https://raw.githubusercontent.com/izhiwen/AgentScienceLab/main/install.sh' README.md || {
  echo "::error::README.md must use raw GitHub install URL"
  exit 1
}
grep -q 'https://raw.githubusercontent.com/izhiwen/AgentScienceLab/main/install.sh' README.zh-CN.md || {
  echo "::error::README.zh-CN.md must use raw GitHub install URL"
  exit 1
}
grep -q 'https://raw.githubusercontent.com/izhiwen/AgentScienceLab/main/install.ps1' README.md || {
  echo "::error::README.md must use raw GitHub Windows install URL"
  exit 1
}
grep -q 'https://raw.githubusercontent.com/izhiwen/AgentScienceLab/main/install.ps1' README.zh-CN.md || {
  echo "::error::README.zh-CN.md must use raw GitHub Windows install URL"
  exit 1
}
if grep -R -n 'asl\.zhiwen-wang\.com' README.md README.zh-CN.md install.sh tests; then
...
echo "ASL_PUBLIC_BRANDING_TEST=PASS"
