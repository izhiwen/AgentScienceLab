...
`agentsciencelab` installs a permanent virtual team of AI research
...
> its memory across unrelated tasks. AgentScienceLab replaces that
...
`agentsciencelab` is the role-and-execution layer for research.
...
## 9. Coordinator (lives inside PI) + ASL Consultant Team
...
Research-specific scoring signals (in `consultant-team.agentsciencelab.toml`):
...
### 9.2 ASL replaces the default SWE consultant team
...
Privacy / Pitfall & Risk / AI Integration). When ASL is installed, it
**replaces** that default with `consultant-team.agentsciencelab.toml`, designed
from first principles for applied-economics research at plan time.

**The ASL consultant is a strategic review board, not a daily team.**
...
If both `aiplus-agent-team` (SWE) and `agentsciencelab` are installed in the
same project, v0.1 takes a simple stance: **ASL install overwrites the
SWE consultant config**. v0.2 will support side-by-side configs
(`consultant-team.swe.toml` + `consultant-team.agentsciencelab.toml`), with the
active config selected by the calling role's tier (PI uses ASL config;
CEO uses SWE config).
...
```
                  agentsciencelab             ← orchestration layer
                           ↓ uses
...
- ASL-tuned consultant team (`consultant-team.agentsciencelab.toml`)
...
- Side-by-side coexistence of `consultant-team.swe.toml` and
  `consultant-team.agentsciencelab.toml` in the same project, with the active
...
| Replace SWE consultant team, not rename SWE seats | yes | The 5 plan-time concerns of econ research (Design Credibility, Contribution Framing, Day-1 Reproducibility, IRB Gate, LLM-as-Measurement) don't map 1:1 to SWE concerns (Architecture, UX, Security, Pitfall, AI Integration). Direct rename would have been duplication of existing ASL roles. Full re-derivation produces seats with named `output_artifact` distinct from execution-time deliverables |
| LIGHT tier skips consult in ASL | yes | ASL consult cadence is ~5-15 per paper per year (review board), not daily. SWE's LIGHT tier of "1 member async" creates noise on routine econ tasks (cluster change, table polish) that Theorist / Replicator already handle |
| Add LLM-as-Measurement as 12th expert | yes | The user's research style and an emerging class of econ papers use LLMs as measurement instruments on text data. Validity protocol is a domain that existing ASL roles do not own; deserves a dedicated expert paired with a consultant seat (same pattern as AI Integration in SWE) |
...
7. The `consultant-team.agentsciencelab.toml` exists, parses, declares all 5
